...
/**
 * Validate parameters
 * @param {*} params - List of Kinvey data rows
 */
...
function formatWorkItemConfig(inArgs, resolution, jobFolder) {
	...
	switch (resolution) {
	case 0:
		config.ActivityId = 'FF_v2-7_Activity';		// hi res, combo directset/ilogic, mm
		break;
	case 1:
		config.ActivityId = 'FF_v2-8_Activity';		// med res, combo directset/ilogic, mm
		break;
	default:
		config.ActivityId = 'FF_v2-8_Activity';		// med res, combo directset/ilogic, mm
		break;
...
try {
	...
	removeFlags()
	if (args.length < 1) {
		console.log('ERROR: Not enough arguments. Must specify Kinvey parameters file [and optional job name].');
		...
		validParams.forEach((rowID) => {
			...
			// // Download STL?
			// const stlFlag = PQ_CONFIG.GET_STL || process.env.GET_STL;
			// // Output file name
